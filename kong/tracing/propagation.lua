...
local AWS_TRACE_ID_DELIMITER = "-"
local AWS_TRACE_ID_DELIMITER_IDX1 = 1
local AWS_TRACE_ID_DELIMITER_IDX2 = 10
...
local function parse_aws_headers(aws_header)
  ...
  for _, key_pair in ipairs(split(aws_header, AWS_KV_PAIR_DELIM)) do
    ...
    if key == AWS_TRACE_ID_KEY then
      if #value ~= AWS_TRACE_ID_LEN
      or sub(value, 1, #AWS_TRACE_ID_VERSION) ~= AWS_TRACE_ID_VERSION
      or sub(value, AWS_TRACE_ID_DELIMITER_IDX1 + 1, AWS_TRACE_ID_DELIMITER_IDX1 + 1) ~= AWS_TRACE_ID_DELIMITER
      or sub(value, AWS_TRACE_ID_DELIMITER_IDX2 + 1, AWS_TRACE_ID_DELIMITER_IDX2 + 1) ~= AWS_TRACE_ID_DELIMITER then
        warn("invalid aws header trace id; ignoring.")
        return nil, nil, nil
      end
      local timestamp_subset = sub(value, AWS_TRACE_ID_DELIMITER_IDX1 + 2, AWS_TRACE_ID_DELIMITER_IDX2)
      local unique_id_subset = sub(value, AWS_TRACE_ID_DELIMITER_IDX2 + 2, #value)
...
local function set(conf_header_type, found_header_type, proxy_span, conf_default_header_type, reuse)
  ...
  if conf_header_type == "aws" or found_header_type == "aws" then
    ...
    set_header("x-amzn-trace-id", fmt("Root=1-%s-%s;Parent=%s;Sampled=%s",
        sub(trace_id, 1, 8),
        sub(trace_id, 9, #trace_id),
        to_hex(proxy_span.span_id),
        proxy_span.should_sample and "1" or "0"))
  end
